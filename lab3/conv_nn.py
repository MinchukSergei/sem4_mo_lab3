...
class ConvNN:
    ...
    def build_model(self):
        ...
        layer1 = conv_layer(to_nwhc, 1, 16, [5, 5], [1, 1], 'HCNV0', self.drop_rate)
        # layer2 = conv_layer(layer1, 16, 16, [5, 5], [2, 2], 'HCNV1', self.drop_rate)
        layer2 = max_pool(layer1, [2, 2])

        flatten = tf.reshape(layer2, [-1, 14 * 14 * 16])

        layer3 = full_connected_layer(flatten, 512, 'HFC1', self.drop_rate)

        self.logits = full_connected_layer(layer3, self.out_size, 'OFC1', self.drop_rate * 0, output=True)

        l2_reg = 0.001
        loss1 = tf.nn.l2_loss(get_tensor_by_name('W_HCNV0')) * l2_reg
        # loss2 = tf.nn.l2_loss(get_tensor_by_name('W_HCNV1')) * l2_reg

        loss3 = tf.nn.l2_loss(get_tensor_by_name('W_HFC1')) * l2_reg

        regularizer = loss1 + loss3
...
def conv_layer(input_data, num_input_channels, num_filters, filter_shape, strides, name, dropout=0):
    conv_filt_shape = [filter_shape[0], filter_shape[1], num_input_channels, num_filters]

    W = weight_variable(name, conv_filt_shape)
    b = bias_variable(name, [num_filters])

    out_layer = tf.nn.conv2d(input_data, W, [1, *strides, 1], padding='SAME') + b
